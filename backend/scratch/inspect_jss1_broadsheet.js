...
async function run() {
  const classId = "P2SWpXEL3F6lImacvkqU"; // JSS 1 (from activity logs)
  ...
  console.log(`Class: ${cls.name} | level: ${cls.level}`);
  console.log(`Total students: ${students.length}`);
  console.log(`Total scores in class: ${scores.length}`);

  // Find IRS JSS scores in JSS 1
  const irsScores = scores.filter(s => s.subjectId === "FIfbYI2YmfYx3JXoN0cW");
  console.log(`IRS JSS scores in database: ${irsScores.length}`);
  ...
  console.log("\nSample students in JSS 1 with Religious Studies scores:");
  sheet.students.slice(0, 5).forEach(st => {
    console.log(`Student: ${st.firstName} ${st.lastName}`);
    console.log(`  Religious Studies score:`, JSON.stringify(st.scores["Ac3cReITFXwiTMLHHuyh"] || st.scores["FIfbYI2YmfYx3JXoN0cW"]));
  });
}
